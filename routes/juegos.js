...
router.get("/", autenticacion, (req, res) => {
  Juego.find()
    .then((resultado) => {
      res.render("juegos_listado", { juegos: resultado });
    })
    .catch((error) => {
...
router.get("/:id", autenticacion, (req, res) => {
  Juego.findById({ _id: req.params["id"] })
    .then((resultado) => {
      res.render("juegos_ficha", { juego: resultado });
    })
    .catch((error) => {
      res.render({error: "Juego no encontrado"});
    });
});
...
router.delete("/juegos/:id/ediciones/:idEdicion", autenticacion, (req, res) => {
    ...
    Juego.findById(id, (error, juego) => {
        // Si el juego se encuentra, borrar la edición y guardar el juego
